...
int qsfp_write_output(const struct device *bus, uint8_t cage, uint8_t value)
{
	...
	/*
	 * An expander that never took QSFP_CONFIG_DIR still ACKs writes to
	 * OUTPUT while leaving every pin an input, so LPMODE/ResetL would be
	 * reported as applied without ever reaching the module.
	 */
