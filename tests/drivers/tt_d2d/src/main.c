...
#ifdef CONFIG_TT_D2D_FW_BLOB
/*
 * The firmware image is not compiled into the driver: it is a separate file,
 * linked in by the build and handed to the loader as a plain buffer. This is
 * the whole of that path, from the linker symbols through to SRAM, so a drop
 * that fails to link or lands truncated fails here rather than as a link that
 * will not train much later.
 *
...
ZTEST(tt_d2d, test_started_firmware_executes)
{
	...
	STEP_RESULT(hit);
	zassert_equal(hit, IMAGE_MAX, "SRAM +0x%zx is already non-zero with the Rocket still in "
